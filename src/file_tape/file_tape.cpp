...
file_tape::file_tape(const std::string &file_name, const Config &config)
...
void file_tape::move_to_start() {
  ...
  size_t delay = static_cast<size_t>(steps * config.rewind_delay_per_position);
  delay(delay);
  current_pos = 0;
}
...
bool file_tape::is_at_end() const {
  file.seekg(0, std::ios::end);
  return current_pos >= file.tellg();
}
